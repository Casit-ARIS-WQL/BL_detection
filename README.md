...
- **中心扩张检测**：从亮区中心出发逐步扩张 mask，按与中心灰度差进行分支停止
...
└─────────────────────┘
    ...
    ▼
┌────────────────────────────┐
│ 2. 中心种子 mask 渐进扩张  │  与中心灰度差超阈值即停止该分支扩张
└────────────────────────────┘
    │
    ▼
┌─────────────────────┐
│ 3. 轮廓与矩形拟合   │  对扩张结果求轮廓并计算最小外接矩形
└─────────────────────┘
    │
    ▼
┌─────────────────────┐
│ 4. 精修与验证       │  Hough/边缘精修 + 面积/矩形度/宽高比校验
└─────────────────────┘
    │
    ▼
┌─────────────────────┐
│ 5. 回退策略         │  扫描线 / 高阈值 / Otsu / 自适应阈值
└─────────────────────┘
    │
    ▼
